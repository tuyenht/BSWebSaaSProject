...
const EcommerceAddProduct = () => {
  ...
  const choicesRef = useRef<HTMLInputElement | null>(null);
  ...
  useEffect(() => {
    if (choicesRef.current) {
      ...
      choicesRef.current.addEventListener("change", (event: any) => {
        const tags = event.detail.value
          .split(",")
          .map((tag: string) => tag.trim())
          .filter((tag: string) => tag.length > 0);
        validation.setFieldValue("product_tags", tags);
      });
    }
                                ...
                                <Col className="col-auto">
                                  {f.preview && (
                                    <Image
                                      src={f.preview}
                                      height={80}
                                      width={80} // added width to avoid layout shift
