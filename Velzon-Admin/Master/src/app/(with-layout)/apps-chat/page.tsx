...
const userDummyImage = "/images/users/user-dummy-img.jpg";
...
interface chatContactDataTye {
  id: number;
  title: string;
  contacts?: (contact & { image?: string | StaticImageData })[];
}
type UserMessage = {
  id: number;
  from_id: number;
...
const Chat = () => {
  const userChatShow: any = useRef<HTMLDivElement>(null);
                        ...
                        >
                          {(chats || []).map(
                            (chatContact: chatContactType) =>
                              ...
                              (chatContact.direactContact || [])?.map(chat => (
                                <li
                          ...
                          <SimpleBar ref={chatRef} style={{ height: "100%" }}>
                            ...
                            >
                              {(
                                ...
                                []
                              )?.map((message: userMessagesType) =>
                                (
                                  (Array.isArray(message.usermessages) &&
                                    message.usermessages) ||
                                  []
                                )?.map((userChat: UserMessage, key: number) => (
                                  <li
                                    className={
                                      userChat.to_id === 1
                                        ? "chat-list left"
