...
import UpcommingEvents from "@components/Calendar/UpcommingEvents";
...
const Calender = () => {
  ...
  const calendarDataProperties = createSelector(
    ...
    (state: any) => ({
      ...
      upcomingEvents: state.upcommingevents,
      isEventUpdated: state.isEventUpdated,
    })
  );
  // Inside your component
  const { events, categories, upcomingEvents, isEventUpdated } = useSelector(
  ...
  const submitOtherEvent = () => {
    ...
    if (eventStartDate) {
      (eventStartDate.parentNode as HTMLElement).classList.remove("d-none");
      const eventLocation = document.getElementById("event-location");
      if (eventLocation) {
        eventLocation.classList.replace("d-none", "d-block");
      }
      const eventDescription = document.getElementById("event-description");
      if (eventDescription) {
        eventDescription.classList.replace("d-none", "d-block");
      }
      const eventStartDateTag = document.getElementById("event-start-date-tag");
      if (eventStartDateTag) {
        eventStartDateTag.classList.replace("d-block", "d-none");
      }
      const eventLocationTag = document.getElementById("event-location-tag");
      if (eventLocationTag) {
        eventLocationTag.classList.replace("d-block", "d-none");
      }
      const eventDescriptionTag = document.getElementById(
        "event-description-tag"
      );
      if (eventDescriptionTag) {
        eventDescriptionTag.classList.replace("d-block", "d-none");
      }
  ...
  return (
    <>
              ...
              <Row>
                      ...
                      <div id="upcoming-event-list">
                        {Array.isArray(upcomingEvents) &&
                          upcomingEvents.map((evt: any, idx: number) => (
                            <UpcommingEvents key={idx} event={evt} />
                          ))}
                      </div>
                    </SimpleBar>
                  </div>
                ...
                <Col xl={9}>
                  <Card className="card-h-100">
                    <CardBody>
                      <FullCalendar
                        // attach ref to access API for custom buttons
                        ref={calendarRef}
                        plugins={[
                          BootstrapTheme,
                          dayGridPlugin,
                        ...
                        // custom navigation buttons using simple text icons
                        customButtons={{
                          prevIcon: {
                            // use simple less-than sign
                            text: "<",
                        ...
                        headerToolbar={{
                          // use custom icon buttons instead of default text buttons
                          left: "prevIcon,nextIcon today",
                          center: "title",
      ...
      <style jsx global>{`
        /* Reset to default background; only change border color */
        .fc-prevIcon-button,
        .fc-nextIcon-button {
          border-color: #364474 !important;
        }
      `}</style>
    </>
  );
